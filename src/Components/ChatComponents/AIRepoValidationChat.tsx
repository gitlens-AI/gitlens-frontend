...
const AIRepoValidationChat = observer(() => {
    const { globalLoading, repoValidationResult } = useContext(Context);

    const copyHandler = () => {
        if (repoValidationResult) {
            copy(repoValidationResult);
        }
    };

    return (
        <div className="chat-container ai-repo-validation-chat">
            {globalLoading ? (
                <p>Loading ...</p>
            ) : (
                <>
                    <ReactMarkdown className={"result-container"}>{repoValidationResult}</ReactMarkdown>
...
export default AIRepoValidationChat;
